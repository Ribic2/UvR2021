...
def razdalja(x: Koordinate, y: Koordinate) -> float:
    ...
    for i in range(len(x)):
        razdalja += pow(x[i]-y[i], 2)
    razdaljak = math.sqrt(razdalja)
    return razdaljak
...
def najblizja(marsovec: Koordinate, ladje: Flota) -> Koordinate:
    ...
    """
    najblizjar=10000000000000000
    najblizjal=0
    for i in ladje:
        if razdalja(marsovec, i) < najblizjar:
            najblizjar = razdalja(marsovec, i)
            najblizjal = i
    return najblizjal
...
def dodeli_ladje(marsovci: Flota, ladje: Flota) -> Flota:
    ...
    for marsovec in marsovci:
        seznam.append(najblizja(marsovec,ladje))
    return seznam
...
def skupine_marsovcev(marsovci: Flota, ladje: Flota) -> Skupine:
    ...
    for i in range(len(marsovci)):
        slovar[marsovci[i]] = cnvl[i]
    for j in ladje:
        mno = set()
        for i,l in slovar.items():
            if j == l:
                mno.add(i)
        #print(mno)
        if mno != set():
...
def optimiraj_ladje(marsovci: Flota, ladje: Flota) -> Flota:
    ...
    i = 0
    skupineold =[]
    skupinenew = skupine_marsovcev(marsovci, ladje)
    while skupinenew != skupineold:
        ladje= razpostavi_ladje(skupinenew)
        skupineold = skupinenew
        skupinenew = skupine_marsovcev(marsovci, ladje)
        #print(ladje)
    #print(i)
    return ladje
...
def kvaliteta(marsovci: Flota, ladje: Flota) -> float:
    ...
    for j in range(len(dodeli_ladje(marsovci,ladje))):
            vsota += razdalja(marsovci[j],dodeli_ladje(marsovci,ladje)[j])
    return vsota
...
def nakljucna(marsovci: Flota, ladij: int) -> Flota:
    ...
    s =[]
    for i in  range(ladij):
        nak = []
        for j in range(len(marsovci[0])):
            nak.append(marsovci[random.randint(0,len(marsovci)-1)][j])
        s.append(tuple(nak))
    #print(s)
    #print(marsovci)
    return s
